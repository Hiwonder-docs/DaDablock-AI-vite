...
const _sfc_main$m = /* @__PURE__ */ defineComponent({
  ...
  setup(__props) {
    const VPLocalSearchBox = defineAsyncComponent(() => __vitePreload(() => import("./VPLocalSearchBox.Czq7l-Dp.js"), true ? __vite__mapDeps([0,1]) : void 0));
...
const _sfc_main = /* @__PURE__ */ defineComponent({
  ...
  setup(__props) {
    const { frontmatter } = useData$1();
    const route = useRoute();
    const DOCUMENTATION_VERSIONS = ["latest", "starter-kit", "standard-kit", "ultimate-kit"];
    const DOCUMENTATION_VERSION_LABELS = {
      latest: "Latest",
      "starter-kit": "Starter Kit",
      "standard-kit": "Standard Kit",
      "ultimate-kit": "Ultimate Kit"
    };
    function detectDocumentationVersion() {
      ...
      return DOCUMENTATION_VERSIONS[0];
    }
    const currentDocumentationVersion = computed(() => {
      void route.path;
      return detectDocumentationVersion();
    });
    function handleMobileVersionChange(event) {
      if (!inBrowser) return;
      const targetVersion = event.target.value;
      const current = detectDocumentationVersion();
    ...
    function injectVersionSwitcher() {
      ...
      const labels = DOCUMENTATION_VERSION_LABELS;
      const current = detectDocumentationVersion();
      ...
      li.querySelectorAll(".version-switcher__item").forEach((item) => {
        item.addEventListener("click", () => {
          ...
          if (ver !== current) {
            ...
            if (ver === "latest") {
              newPath = newPath.replace(/\/en\/latest\/.*$/, "/en/latest/docs/index.html");
            }
          ...
          "nav-screen-content-after": withCtx(() => [
            createBaseVNode("div", _hoisted_1, [
              ...
              createBaseVNode("select", {
                ...
                value: currentDocumentationVersion.value,
                "aria-label": "Documentation version",
                onChange: handleMobileVersionChange
              }, [
                (openBlock(), createElementBlock(Fragment, null, renderList(DOCUMENTATION_VERSIONS, (version) => {
                  return createBaseVNode("option", {
                    key: version,
                    value: version
                  }, toDisplayString(DOCUMENTATION_VERSION_LABELS[version]), 9, _hoisted_3);
                }), 64))
              ], 40, _hoisted_2)
            ])
          ]),
